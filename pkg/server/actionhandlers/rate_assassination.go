...
func ApplyRateAssassination(ctx context.Context, tx store.QueryRower, gameID int, action GameAction) error {
	...
	game.Phase = constants.STATE_ASSASSINATION
	game.TurnsOrder = []int{assassinPos}
	game.SpeakerPosition = assassinPos
	return store.UpdateGame(ctx, tx, game)
}
